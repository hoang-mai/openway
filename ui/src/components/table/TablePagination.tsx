...
import { DEFAULT_PAGE_SIZE_OPTIONS, tablePaginationConfig } from "./constants";
...
export function TablePagination<TData extends RowData = RowData>({
  ...
  return (
    <div
      className={`${tablePaginationConfig.container} ${className}`.trim()}
    >
      ...
      <div className="flex items-center gap-1 ml-auto sm:ml-0">
        ...
        {/* Các số trang hiển thị dạng chữ nhật bo góc chuẩn Notion */}
          ...
          {paginationRange.map((item, index) => {
            if (item === "...") {
              return (
                <span
                  key={`ellipsis-${index}`}
                  className={tablePaginationConfig.ellipsis}
                >
                  ...
                </span>
              );
            ...
            return (
              <button
                ...
                className={`${tablePaginationConfig.pageButton} ${
                  isCurrent
                    ? tablePaginationConfig.pageButtonActive
                    : tablePaginationConfig.pageButtonInactive
                }`}
              >
                {pageNum}
              </button>
